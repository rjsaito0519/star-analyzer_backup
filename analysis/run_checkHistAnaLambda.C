// run_checkHistAnaLambda.C - Load libStarAnaConfig, compile checkHistAnaLambda.C+, and call it.
// ROOT's CINT does not resolve symbols from dynamically loaded libs; we must compile with ACLiC and link.
// Usage: root4star -b -q 'run_checkHistAnaLambda.C("rootfile/...","anaName","config/mainconf/main_anaName.yaml")'

void run_checkHistAnaLambda(const Char_t* rootFile,
                            ...
                            const Char_t* mainconfPath = 0)
{
  ...
  gROOT->ProcessLine(TString::Format(".L %s/common/macro/checkHistAnaLambda.C+", pwd));
  checkHistAnaLambda(rootFile, anaName, mainconfPath);
}
