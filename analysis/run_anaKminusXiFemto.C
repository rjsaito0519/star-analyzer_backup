// run_anaKminusXiFemto.C - Wrapper to load libraries and run K--Xi- Femtoscopy
// Usage: root4star -b -q 'run_anaKminusXiFemto.C("input.list","output.root","0",100,"config/mainconf/main_auau19_anaKminusXiFemto.yaml")'

void run_anaKminusXiFemto(const Char_t* inputFile,
                         ...
                         const Char_t* configPath = 0)
{
  ...
  if (gSystem->Load(TString(pwd) + "/lib/libStKminusXiFemtoMaker.so") < 0) {
    std::cerr << "ERROR: failed to load libStKminusXiFemtoMaker.so" << std::endl;
    return;
  }
  ...
  gSystem->AddLinkedLibs(TString::Format(
      "-L%s/lib -lStarAnaConfig -lStRefMultCorr -lStCommon -lStXiMaker -lStKminusXiFemtoMaker -Wl,-rpath,%s/lib", pwd,
      pwd));

  gROOT->ProcessLine(TString::Format(".L %s/analysis/anaKminusXiFemto.C+", pwd));
  anaKminusXiFemto(inputFile, outputFile, jobid, nEventsMax, configPath);
}
