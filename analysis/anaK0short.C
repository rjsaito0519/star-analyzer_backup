// anaK0short.C - StChain based phi analysis macro
// Usage: root4star -b -q 'anaK0short.C("input.list","output.root","0",-1)'
//        anaK0short.C("input.list","output.root","0",-1,"config/mainconf/main_auau19_anaK0short.yaml")
// Run from project root: ./script/run_anaK0short.sh
// ACLiC (.L anaK0short.C+) links against libStK0shortMaker for StK0shortMaker
...
#include "StMaker/StK0shortMaker/StK0shortMaker.h"
#include "ConfigManager.h"
#include <iostream>
#include <fstream>

StChain* chain = 0;
StK0shortMaker* k0shortMaker = 0;

void anaK0short(const Char_t* inputFile = "config/picoDstList/auau19GeV.list",
            const Char_t* outputFile = "rootfile/auau19_anaK0short_temp/auau19_anaK0short_temp.root",
            ...
            const Char_t* configPath = 0)
{
  ...
  // `run_anaK0short.C` loads the STAR/Pico/Maker libraries before ACLiC-compiling
  ...
  if (configPath && strlen(configPath) > 0) {
    ...
    if (env_conf && strlen(env_conf) > 0) {
      ...
      if (infile.is_open() && std::getline(infile, line)) {
        ...
        mainConfigPath = TString(pwd) + "/config/mainconf/main_auau19_anaK0short.yaml";
      }
    }
  }
  ...
  k0shortMaker = new StK0shortMaker("k0short", picoMaker, outputFile);
  chain->AddMaker(picoMaker);
  chain->AddMaker(k0shortMaker);
