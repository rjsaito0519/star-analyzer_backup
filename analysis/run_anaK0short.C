// run_anaK0short.C - Wrapper to load lib and call anaK0short
// ROOT -q treats only one macro; load STAR libs first, then libStarAnaConfig, then libStK0shortMaker.
// Usage: root4star -b -q 'run_anaK0short.C("input.list","output.root","0",100)'
//        run_anaK0short.C("input.list","output.root","0",100,"config/mainconf/main_auau19_anaK0short.yaml")'

void run_anaK0short(const Char_t* inputFile,
                ...
                const Char_t* configPath = 0)
{
  ...
  gSystem->AddLinkedLibs(TString::Format("-L%s/lib -lStarAnaConfig -lStRefMultCorr -lStCommon -lStK0shortMaker -Wl,-rpath,%s/lib", pwd, pwd));

  gROOT->ProcessLine(TString::Format(".L %s/analysis/anaK0short.C+", pwd));
  anaK0short(inputFile, outputFile, jobid, nEventsMax, configPath);
}
