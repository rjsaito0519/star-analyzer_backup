// run_anaXi.C - Wrapper to load lib and call anaXi
// ROOT -q treats only one macro; load STAR libs first, then libStarAnaConfig, then libStXiMaker.
// Usage: root4star -b -q 'run_anaXi.C("input.list","output.root","0",100)'
//        run_anaXi.C("input.list","output.root","0",100,"config/mainconf/main_auau19_anaXi.yaml")'

void run_anaXi(const Char_t* inputFile,
                ...
                const Char_t* configPath = 0)
{
  ...
  gSystem->AddLinkedLibs(TString::Format("-L%s/lib -lStarAnaConfig -lStRefMultCorr -lStCommon -lStXiMaker -Wl,-rpath,%s/lib", pwd, pwd));

  gROOT->ProcessLine(TString::Format(".L %s/analysis/anaXi.C+", pwd));
  anaXi(inputFile, outputFile, jobid, nEventsMax, configPath);
}
