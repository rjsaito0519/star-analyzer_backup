...
TString XiKfQuote(const char* value) {
...
void run_anaXi_KFParticle(const Char_t* inputFile, const Char_t* outputFile,
  ...
  const char* libraries[] = {"libStarAnaConfig.so", "libStRefMultCorr.so", "libKFParticle.so",
      "libStKfParticleCommon.so", "libStCommon.so", "libStXiKFParticleMaker.so"};
  ...
  gSystem->AddLinkedLibs(TString::Format(
      "-L%s/lib -lStXiKFParticleMaker -lStKfParticleCommon -lKFParticle "
  ...
  if (!gSystem->CompileMacro((pwd + "/analysis/anaXi_KFParticle.C").Data(), "kf")) {
    std::cerr << "ERROR: compiling anaXi_KFParticle.C" << std::endl;
    gSystem->Exit(1); return;
  }
  const TString call = TString::Format("anaXi_KFParticle(%s,%s,%s,%lld,%s)",
      XiKfQuote(inputFile).Data(), XiKfQuote(outputFile).Data(),
      XiKfQuote(jobid).Data(), nEventsMax, XiKfQuote(configPath).Data());
  Int_t error = 0;
