// anaXiFxt.C - StChain FXT Xi analysis (StXiFxtMaker)
// Energy-specific numbers live in mainconf YAML, not in this macro.
...
void anaXiFxt(const Char_t* inputFile = "config/picoDstList/auau3p9fxt_test.list",
              const Char_t* outputFile = "rootfile/auau3p9fxt_anaXi_temp/auau3p9fxt_anaXi_temp.root",
              ...
              const Char_t* configPath = 0)
{
  ...
  if (configPath && strlen(configPath) > 0) {
    ...
    if (env_conf && strlen(env_conf) > 0) {
      ...
      if (infile.is_open() && std::getline(infile, line)) {
        ...
        mainConfigPath = TString(pwd) + "/config/mainconf/main_auau3p9fxt_anaXi.yaml";
      }
    }
  }
  ...
  xiMaker = new StXiFxtMaker("xiFxt", picoMaker, outputFile);
