// anaXi.C - StChain based phi analysis macro
// Usage: root4star -b -q 'anaXi.C("input.list","output.root","0",-1)'
//        anaXi.C("input.list","output.root","0",-1,"config/mainconf/main_auau19_anaXi.yaml")
// Run from project root: ./script/run_anaXi.sh
// ACLiC (.L anaXi.C+) links against libStXiMaker for StXiMaker
...
void anaXi(const Char_t* inputFile = "config/picoDstList/auau19GeV.list",
            const Char_t* outputFile = "rootfile/auau19_anaXi_temp/auau19_anaXi_temp.root",
            ...
            const Char_t* configPath = 0)
{
  ...
  // `run_anaXi.C` loads the STAR/Pico/Maker libraries before ACLiC-compiling
  ...
  if (configPath && strlen(configPath) > 0) {
    ...
    if (env_conf && strlen(env_conf) > 0) {
      ...
      if (infile.is_open() && std::getline(infile, line)) {
        ...
        mainConfigPath = TString(pwd) + "/config/mainconf/main_auau19_anaXi.yaml";
      }
    }
  }
  ...
  xiMaker = new StXiMaker("xi", picoMaker, outputFile);
