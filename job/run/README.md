...
2. **Move to this directory and submit**
   ...
   ```
   Pass the joblist XML you want (for example the file produced by `./script/generate_joblist.sh`, named **`joblist_<anaName>.xml`** where **`anaName`** is `analysis.anaName` in your analysis_info). By default, `submit.sh` uses **`../joblist/joblist_auau19_anaLambda_temp.xml`** if you omit the argument (adjust the default in `submit.sh` if your primary analysis differs). Example for Phi: `./submit.sh ../joblist/joblist_auau19_anaPhi.xml`. Example for K⁺–Ξ⁻ / K⁻–Ξ⁻ femto: generate/submit `joblist_auau19_anaKplusXiFemto.xml` or `joblist_auau19_anaKminusXiFemto.xml`.
