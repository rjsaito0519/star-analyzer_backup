#!/bin/bash
# Run anaK0XiFxtFemto.C - FXT K0s–Xi femtoscopy
# Usage: ./script/run_anaK0XiFxtFemto.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaK0XiFxtFemto.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaK0XiFxtFemto_C.* 2>/dev/null
