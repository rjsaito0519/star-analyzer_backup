#!/bin/bash
# Run anaKminusXiFemto.C - K--Xi- femtoscopy
# Usage: ./script/run_anaKminusXiFemto.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaKminusXiFemto.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaKminusXiFemto_C.* 2>/dev/null
root4star -b -q "analysis/run_anaKminusXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$MAINCONF\")"
