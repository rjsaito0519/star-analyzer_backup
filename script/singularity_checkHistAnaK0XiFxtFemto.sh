#!/bin/bash
# Run checkHistAnaK0XiFxtFemto.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_checkHistAnaK0XiFxtFemto.sh <root_file> <mainconf_path>
...
if [[ -z "$ROOT_FILE" || -z "$MAINCONF" ]]; then
  ...
  echo "  root_file     ROOT file produced by run_anaK0XiFxtFemto.C" >&2
  echo "  mainconf_path main config (required, e.g. config/mainconf/main_auau19_anaK0XiFxtFemto.yaml)" >&2
  exit 1
fi
...
root4star -b -q 'analysis/run_checkHistAnaK0XiFxtFemto.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
