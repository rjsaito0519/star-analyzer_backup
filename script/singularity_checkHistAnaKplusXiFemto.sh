#!/bin/bash
# Run checkHistAnaKplusXiFemto.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_checkHistAnaKplusXiFemto.sh <root_file> <mainconf_path>
...
if [[ -z "$ROOT_FILE" || -z "$MAINCONF" ]]; then
  ...
  echo "  root_file     ROOT file produced by run_anaKplusXiFemto.C" >&2
  echo "  mainconf_path main config (required, e.g. config/mainconf/main_auau19_anaKplusXiFemto.yaml)" >&2
  exit 1
fi
...
rm -f "$PROJECT_ROOT_REAL"/common/macro/checkHistAnaKplusXiFemto_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_checkHistAnaKplusXiFemto.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
