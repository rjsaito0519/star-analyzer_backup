#!/bin/bash
# Run anaXiFxt.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaXiFxt.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
if [[ -z "$MAINCONF" ]]; then
  ...
  echo "  MAINCONF    main config (required, e.g. config/mainconf/main_auau3p9fxt_anaXi.yaml)" >&2
  ...
  echo "  jobid       job id passed to anaXiFxt (default: 0)" >&2
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaXiFxt_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_anaXiFxt.C("$INPUT_FILE","$OUTPUT_FILE","$JOBID",$NEVENTS,"$MAINCONF")'
EOF
)
