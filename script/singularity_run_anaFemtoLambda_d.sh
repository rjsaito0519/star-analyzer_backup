#!/bin/bash
# Run anaFemtoLambda_d.C inside singularity (batch-like environment).
# Usage: ./script/singularity_run_anaFemtoLambda_d.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaFemtoLambda_d_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_anaFemtoLambda_d.C("$INPUT_FILE_REAL","$OUTPUT_FILE_REAL","$JOBID",$NEVENTS,"$MAINCONF_REAL")'
EOF
)
