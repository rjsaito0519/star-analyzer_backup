#!/bin/bash
# Run anaFemtoLambda_4He.C - Lambda x 4He femtoscopy with StFemtoMakerLambdaNuclear
# Usage: ./script/run_anaFemtoLambda_4He.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau3p85_anaFemtoLambda_4He.yaml}"
...
rm -f "$PROJECT_ROOT"/analysis/anaFemtoLambda_4He_C.* 2>/dev/null

if [ -n "$CONFIG_PATH" ]; then
  root4star -b -q "analysis/run_anaFemtoLambda_4He.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  root4star -b -q "analysis/run_anaFemtoLambda_4He.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
