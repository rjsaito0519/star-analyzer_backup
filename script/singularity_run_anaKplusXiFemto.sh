...
# Usage: ./script/singularity_run_anaKplusXiFemto.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
OUTPUT_FILE="${2:-rootfile/auau19_anaKplusXiFemto_temp/auau19_anaKplusXiFemto_temp.root}"
JOBID="${3:-0}"
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau19_anaKplusXiFemto.yaml}"
...
echo "=== anaKplusXiFemto.C (singularity) ==="
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaKplusXiFemto_C.* 2>/dev/null
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaKplusXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaKplusXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
