#!/bin/bash
# Run K0-Xi Femtoscopy analysis inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaKXiFemto.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
OUTPUT_FILE="${2:-rootfile/auau19_anaKXiFemto_temp/auau19_anaKXiFemto_temp.root}"
JOBID="${3:-0}"
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau19_anaKXiFemto.yaml}"
...
echo "=== anaKXiFemto.C (singularity) ==="
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaKXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaKXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
