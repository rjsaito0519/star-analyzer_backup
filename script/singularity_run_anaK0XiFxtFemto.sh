#!/bin/bash
# Run anaK0XiFxtFemto.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaK0XiFxtFemto.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
if [[ -z "$MAINCONF" ]]; then
  ...
  echo "  MAINCONF    main config (required, e.g. config/mainconf/main_auau3p9fxt_anaK0XiFemto.yaml)" >&2
  ...
  echo "  jobid       job id passed to anaK0XiFxtFemto (default: 0)" >&2
...
echo "=== anaK0XiFxtFemto.C (singularity) ==="
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaK0XiFxtFemto_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_anaK0XiFxtFemto.C("$INPUT_FILE","$OUTPUT_FILE","$JOBID",$NEVENTS,"$MAINCONF")'
EOF
)
