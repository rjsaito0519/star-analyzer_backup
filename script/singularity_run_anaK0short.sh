#!/bin/bash
# Run anaK0short.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaK0short.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
OUTPUT_FILE="${2:-rootfile/auau19_anaK0short_temp/auau19_anaK0short_temp.root}"
JOBID="${3:-0}"
NEVENTS="${4:--1}"
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau19_anaK0short.yaml}"
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaK0short_C.* 2>/dev/null
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaK0short.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaK0short.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
