#!/bin/bash
# Run anaXi_KFParticle.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaXi_KFParticle.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
INPUT_FILE="${1:-config/picoDstList/auau3p9fxt_test.list}"
OUTPUT_FILE="${2:-rootfile/auau3p9fxt_anaXi_KFParticle_temp/auau3p9fxt_anaXi_KFParticle_temp.root}"
JOBID="${3:-0}"
NEVENTS="${4:--1}"
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau3p9fxt_anaXi_KFParticle.yaml}"
...
echo "=== anaXi_KFParticle.C (singularity) ==="
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaXi_KFParticle.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaXi_KFParticle.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
