#!/bin/bash
# Run checkHistAnaKXiFemto.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_checkHistAnaKXiFemto.sh <root_file> <mainconf_path>
...
if [[ -z "$ROOT_FILE" || -z "$MAINCONF" ]]; then
  ...
  echo "  root_file     ROOT file produced by run_anaKXiFemto.C" >&2
  echo "  mainconf_path main config (required, e.g. config/mainconf/main_auau19_anaKXiFemto.yaml)" >&2
  exit 1
fi
...
root4star -b -q 'analysis/run_checkHistAnaKXiFemto.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
