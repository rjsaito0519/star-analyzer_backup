#!/bin/bash
# Run checkHistAnaXi_KFParticle.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_checkHistAnaXi_KFParticle.sh <root_file> <mainconf_path>
...
if [[ -z "$ROOT_FILE" || -z "$MAINCONF" ]]; then
  ...
  echo "  root_file     ROOT file produced by anaXi_KFParticle" >&2
  echo "  mainconf_path main config (e.g. config/mainconf/main_auau3p9fxt_anaXi_KFParticle.yaml)" >&2
  exit 1
fi
...
rm -f "$PROJECT_ROOT_REAL"/common/macro/checkHistAnaXi_KFParticle_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_checkHistAnaXi_KFParticle.C("$ROOT_FILE_REAL","$ANA_NAME","$MAINCONF_REAL")'
EOF
)
