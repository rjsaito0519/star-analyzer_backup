#!/bin/bash
# Run anaKplusXiFemto.C - K+-Xi- femtoscopy
# Usage: ./script/run_anaKplusXiFemto.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaKplusXiFemto.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaKplusXiFemto_C.* 2>/dev/null
root4star -b -q "analysis/run_anaKplusXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$MAINCONF\")"
