#!/bin/bash
# Run anaFemtoLambda_3He.C inside singularity (batch-like environment).
# Usage: ./script/singularity_run_anaFemtoLambda_3He.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaFemtoLambda_3He_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_anaFemtoLambda_3He.C("$INPUT_FILE_REAL","$OUTPUT_FILE_REAL","$JOBID",$NEVENTS,"$MAINCONF_REAL")'
EOF
)
