#!/bin/bash
# Run anaXi.C inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaXi.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
OUTPUT_FILE="${2:-rootfile/auau19_anaXi_temp/auau19_anaXi_temp.root}"
JOBID="${3:-0}"
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau19_anaXi.yaml}"
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaXi_C.* 2>/dev/null
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaXi.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaXi.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
