#!/bin/bash
# Usage: ./script/run_anaPhiMesicNucleus.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaPhiMesicNucleus.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaPhiMesicNucleus_C.* 2>/dev/null
root4star -b -q "analysis/run_anaPhiMesicNucleus.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$MAINCONF\")"
