#!/bin/bash
# Run K--Xi- Femtoscopy analysis inside the same singularity context as batch jobs.
# Usage: ./script/singularity_run_anaKminusXiFemto.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
OUTPUT_FILE="${2:-rootfile/auau19_anaKminusXiFemto_temp/auau19_anaKminusXiFemto_temp.root}"
JOBID="${3:-0}"
NEVENTS="${4:-1000}" # Default to 1000 events for debugging
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau19_anaKminusXiFemto.yaml}"
...
echo "=== anaKminusXiFemto.C (singularity) ==="
input_tail=$(tail -n 2 "$INPUT_FILE_REAL" | xargs || true)
echo "Input:   $INPUT_FILE (samples: $input_tail)"
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaKminusXiFemto_C.* 2>/dev/null
...
collect_bind_mounts() {
  ...
  add_path_binds() {
    ...
    if [[ "$candidate" == /star/data* ]]; then
      ...
      add_bind "/direct"
    elif [[ "$candidate" == /star/u/* ]]; then
      add_bind "/star/u"
    elif [[ "$candidate" == /star/nfs4/* ]]; then
      add_bind "/star/nfs4"
    fi
  }
...
if [[ -n "$CONFIG_PATH" ]]; then
  ROOT4STAR_CMD="analysis/run_anaKminusXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  ROOT4STAR_CMD="analysis/run_anaKminusXiFemto.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
