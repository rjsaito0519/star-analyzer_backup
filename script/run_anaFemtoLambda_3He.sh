#!/bin/bash
# Run anaFemtoLambda_3He.C - Lambda x 3He femtoscopy with StFemtoMakerLambdaNuclear
# Usage: ./script/run_anaFemtoLambda_3He.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau3p85_anaFemtoLambda_3He.yaml}"
...
rm -f "$PROJECT_ROOT"/analysis/anaFemtoLambda_3He_C.* 2>/dev/null

if [ -n "$CONFIG_PATH" ]; then
  root4star -b -q "analysis/run_anaFemtoLambda_3He.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  root4star -b -q "analysis/run_anaFemtoLambda_3He.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
