#!/bin/bash
# Usage: ./script/singularity_run_anaPhiMesicNucleus.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
...
rm -f "$PROJECT_ROOT_REAL"/analysis/anaPhiMesicNucleus_C.* 2>/dev/null
...
root4star -b -q 'analysis/run_anaPhiMesicNucleus.C("$INPUT_FILE_REAL","$OUTPUT_FILE_REAL","$JOBID",$NEVENTS,"$MAINCONF_REAL")'
EOF
)
