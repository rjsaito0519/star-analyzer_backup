#!/bin/bash
# Run anaXiFxt.C - FXT Xi with StXiFxtMaker
# Usage: ./script/run_anaXiFxt.sh MAINCONF [inputFile] [outputFile] [jobid] [nEvents]
#   MAINCONF  e.g. config/mainconf/main_auau3p9fxt_anaXi.yaml (required)
...
MAINCONF="${MAINCONF:?Usage: ./script/run_anaXiFxt.sh [MAINCONF] [inputFile] [outputFile] [jobid] [nEvents]}"
...
rm -f "$PROJECT_ROOT"/analysis/anaXiFxt_C.* 2>/dev/null
