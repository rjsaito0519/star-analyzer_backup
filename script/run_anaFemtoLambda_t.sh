#!/bin/bash
# Run anaFemtoLambda_t.C - Lambda x triton femtoscopy with StFemtoMakerLambdaNuclear
# Usage: ./script/run_anaFemtoLambda_t.sh [inputFile] [outputFile] [jobid] [nEvents] [configPath]
...
SETUP_MAINCONF="${CONFIG_PATH:-config/mainconf/main_auau3p85_anaFemtoLambda_t.yaml}"
...
rm -f "$PROJECT_ROOT"/analysis/anaFemtoLambda_t_C.* 2>/dev/null

if [ -n "$CONFIG_PATH" ]; then
  root4star -b -q "analysis/run_anaFemtoLambda_t.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS,\"$CONFIG_PATH\")"
else
  root4star -b -q "analysis/run_anaFemtoLambda_t.C(\"$INPUT_FILE\",\"$OUTPUT_FILE\",\"$JOBID\",$NEVENTS)"
fi
