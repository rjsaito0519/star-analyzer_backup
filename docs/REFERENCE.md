...
| **StMaker/** | One subdir per Maker (e.g. `StLambdaMaker/`, `StPhiMaker/`, `StFemtoMaker/`, `StKplusXiFemtoMaker/`). Each has `.h` and `.cxx`; built into `lib/libStXXXMaker.so`. Femto naming rules: `StMaker/StFemtoMaker/README.md`. |
...
- **K⁺–Ξ⁻ femto QA:** `./script/singularity_checkHistAnaKplusXiFemto.sh <root_file> <mainconf_path>` — smoke QA (event/K⁺/Ξ/k*).
