...
void checkHistAnaKplusXiFemto(const Char_t* rootFile,
  ...
  TH1* hSE_L = (TH1*)fin->Get("hKstarSE_kp_xim_leftSB");
  TH1* hME_L = (TH1*)fin->Get("hKstarME_kp_xim_leftSB");
  TH1* hSE_R = (TH1*)fin->Get("hKstarSE_kp_xim_rightSB");
  TH1* hME_R = (TH1*)fin->Get("hKstarME_kp_xim_rightSB");
  if (hSE_L || hSE_R) {
    c1->Clear();
    c1->Divide(3, 2);
    c1->cd(1);
  ...
  TH1* hSE_LR = sumTwoHists(hSE_L, hSE_R, "hSE_kp_xim_SBLR");
  TH1* hME_LR = sumTwoHists(hME_L, hME_R, "hME_kp_xim_SBLR");
  if (hSE_LR && hME_LR) {
    c1->Clear();
    c1->Divide(2, 2);
    c1->cd(1);
