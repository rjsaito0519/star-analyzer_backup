// checkHistAnaK0short.C - Draw histograms from run_anaK0short.C output and write PDF.
// Invoke via: ./script/singularity_checkHistAnaK0short.sh <root_file> <mainconf_path>
...
void checkHistAnaK0short(const Char_t* inputRootFile,
                         const Char_t* anaNameArg = "auau19_anaK0short",
                         const Char_t* mainconfPath = 0)
{
  ...
  if (gSystem->Load(TString(pwd) + "/lib/libStarAnaConfig.so") >= 0) {
    ...
    if (ConfigManager::GetInstance().LoadConfig(mainconf.Data())) {
      ...
      std::cerr << "[checkHistAnaK0short] WARNING: Failed to load config " << mainconf.Data() << "; cut lines skipped." << std::endl;
    }
  }
  ...
  TString pdfName = TString(outDir) + anaName + "_checkHistAnaK0short";
  ...
  TString note = "Check histograms from run_anaK0short.C (StK0shortMaker output).\n";
  note += "K0short V0 (pi+ + pi-) helix reconstruction.\n";
  note += "Centrality QA: Pages 1b-1d when centrality is enabled in mainconf.\n";

  PdfHeader::MakePdfHeaderPage(pdfName, "checkHistAnaK0short.C", inputs, note.Data(), true, anaName);

  TCanvas* c1 = new TCanvas("c1", "canvas", 1200, 800);
  TH1* h1 = 0;
  TH2* h2 = 0;

  // Page 1: Event level (K0short)
  c1->Clear();
  c1->Divide(2, 2);
  c1->cd(1); h1 = (TH1*)fin->Get("hVz"); if (h1) h1->Draw();
  c1->cd(2); h1 = (TH1*)fin->Get("hRefMult"); if (h1) h1->Draw();
  c1->cd(3); h1 = (TH1*)fin->Get("hK0short_InvMass"); if (h1) h1->Draw();
  ...
  c1->cd(5); h2 = (TH2*)fin->Get("hNPionPosCand_vs_Cent9"); if (h2) h2->Draw("colz");
  c1->cd(6); h2 = (TH2*)fin->Get("hNPionNegCand_vs_Cent9"); if (h2) h2->Draw("colz");
  c1->cd(7); h2 = (TH2*)fin->Get("hNK0shortPairs_vs_Cent9"); if (h2) h2->Draw("colz");
  c1->cd(8); h2 = (TH2*)fin->Get("hK0short_InvMass_vs_Cent9"); if (h2) h2->Draw("colz");
  c1->cd(9); h2 = (TH2*)fin->Get("hK0short_InvMass_vs_RefMultCorr"); if (h2) h2->Draw("colz");
  drawCent9ConventionNote();
  c1->Print(pdfName);

  // Page 1d: K0short InvMass per centrality bin
  c1->Clear();
  c1->Divide(3, 3);
  c1->cd(1); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin0"); if (h1) h1->Draw();
  c1->cd(2); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin1"); if (h1) h1->Draw();
  c1->cd(3); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin2"); if (h1) h1->Draw();
  c1->cd(4); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin3"); if (h1) h1->Draw();
  c1->cd(5); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin4"); if (h1) h1->Draw();
  c1->cd(6); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin5"); if (h1) h1->Draw();
  c1->cd(7); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin6"); if (h1) h1->Draw();
  c1->cd(8); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin7"); if (h1) h1->Draw();
  c1->cd(9); h1 = (TH1*)fin->Get("hK0short_InvMass_CentBin8"); if (h1) h1->Draw();
  drawCent9ConventionNote();
  c1->Print(pdfName);

  // Page 2: K0short kinematics and quality
  c1->Clear();
  c1->Divide(3, 3);
  c1->cd(1); gPad->SetLogy(); h1 = (TH1*)fin->Get("hK0short_Pt"); if (h1) h1->Draw();
  c1->cd(2); h1 = (TH1*)fin->Get("hK0short_Eta"); if (h1) h1->Draw();
  c1->cd(3); h1 = (TH1*)fin->Get("hK0short_Phi"); if (h1) h1->Draw();
  ...
  c1->cd(7); h1 = (TH1*)fin->Get("hNSigmaPionPos"); if (h1) {
    h1->Draw();
    if (gConfigLoaded) {
      LambdaCutConfig& lam = ConfigManager::GetInstance().GetLambdaCuts();
  ...
  c1->cd(3); h2 = (TH2*)fin->Get("hK0short_InvMass_vs_DecayLength"); if (h2) h2->Draw("colz");
  c1->cd(4); h2 = (TH2*)fin->Get("hK0short_InvMass_vs_Y"); if (h2) h2->Draw("colz");
  c1->Print(pdfName);
